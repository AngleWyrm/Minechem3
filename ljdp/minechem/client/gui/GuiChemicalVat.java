...
public class GuiChemicalVat extends GuiContainerTabbed {
	...
	protected void drawGuiContainerForegroundLayer(int i, int j) {
		...
		if(vat.isSpoiled) {
			...
			info = String.format("%s x %d", 
					MinechemHelper.getChemicalName(vat.chemical), 
					vat.amountOfChemical);
		} else {
			info = "Empty";
	...
	protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		int textureId = mc.renderEngine.getTexture(ModMinechem.proxy.VAT_GUI_PNG);
	...
	private void drawStorageBar() {
		...
		storageBarWidth = (int) MinechemHelper.translateValue(vat.amountOfChemical, 
				0, vat.MAX_CHEMICAL_AMOUNT, 
				0, GUI_MAX_STORAGE_WIDTH);

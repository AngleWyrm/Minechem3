...
public class TileEntityFusion extends TileEntityMultiBlock implements
		ISidedInventory, IMinechemMachinePeripheral {
	...
	public TileEntityFusion() {
		inputInventory = new BoundedInventory(this, kStartInput1, kStartInput1
				+ kSizeInput);
		outputInventory = new BoundedInventory(this, kStartOutput, kStartOutput
				+ kSizeOutput);
		starInventory = new BoundedInventory(this, kStartFusionStar,
				kSizeFusionStar + kSizeFusionStar);
	...
	public void updateEntity() {
		...
		if (!worldObj.isRemote
				&& inventory[kStartFusionStar] != null
				&& energyUpdateTracker.markTimeIfDelay(worldObj,
						Constants.TICKS_PER_SECOND * 2)) {
			if (!isRecharging)
				targetEnergy = takeEnergyFromStar(inventory[kStartFusionStar],
						true);
			if (targetEnergy > 0)
				isRecharging = true;
			if (isRecharging) {
	...
	private boolean canFuse(ItemStack fusionResult) {
		...
		if (itemInOutput != null)
			return itemInOutput.stackSize < getInventoryStackLimit()
					&& itemInOutput.isItemEqual(fusionResult)
					&& energyStored >= getEnergyCost(fusionResult);
		else
	...
	private int getEnergyCost(ItemStack itemstack) {
		...
		int cost = (int) MinechemHelper.translateValue(mass + 1, 1,
				EnumElement.heaviestMass, 1, this.maxEnergy);
		return cost;
	}

	private boolean hasInputs() {
		return inventory[kStartInput1] != null
				&& inventory[kStartInput2] != null;
	}
	...
	public void readFromNBT(NBTTagCompound nbtTagCompound) {
		...
		MinechemHelper.readTagListToItemStackArray(
				nbtTagCompound.getTagList("inventory"), inventory);
	}
